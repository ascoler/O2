...
def load_data(filepath):
    ...
    text_columns = ['artistName', 'collectionName', 'trackName', 'primaryGenreName', 'country', 'contentAdvisoryRating']
    ...
    for idx, row in df.iterrows():
        queries.append(f"песня {row.get('trackName', '')} {row.get('artistName', '')}")
        queries.append(f"альбом {row.get('collectionName', '')} {row.get('artistName', '')}")
        
        if 'releaseYear' in df.columns and row.get('releaseYear', 0) > 0:
            queries.append(f"музыка {int(row.get('releaseYear'))} года")
        
        if 'primaryGenreName' in df.columns:
            queries.append(f"{row.get('primaryGenreName')} музыка")
        
        if 'explicit' in df.columns:
            queries.append(f"{row.get('explicit')} трек {row.get('artistName', '')}")
...
def main_retrain():
    filepath = "apple_music_dataset.csv"
